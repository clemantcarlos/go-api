...
func (s *store) GetAll() ([]*model.Book, error){
	...
	for rows.Next(){
		b := model.Book{}
		if err := rows.Scan(&b.ID, &b.Author, &b.Titulo); err != nil {
			return nil, err
		}
...
func (s *store) GetById(id int) (*model.Book, error){
	...
	b := model.Book{}
	err := s.db.QueryRow(q,id).Scan(&b.ID, &b.Titulo, &b.Author)
	if err != nil{
		return nil, err
	}
	return &b, nil
...
func (s *store) Create(book *model.Book) (*model.Book, error){
	...
	resp, err := s.db.Exec(q, book.Titulo, book.Author)
	if err != nil{
		return nil, err
	}
...
func (s *store) Update(id int ,book *model.Book) (*model.Book, error){
	...
	_, err := s.db.Exec(q, book.Titulo, book.Author, id)
	if err != nil {
		return nil, err
	}
